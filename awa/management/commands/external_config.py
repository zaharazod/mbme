...
class Command(BaseCommand):
    ...
    def handle(self, **kw):
        try:
            config = AwaConfig(path=kw["config"]) if kw["config"] else default_config
            ...
            storages = dict(
                [
                    (k, s)
                    for k, s in config.storages.items()
                    if isinstance(s, dict) and s.type == "external"
                ]
            )
